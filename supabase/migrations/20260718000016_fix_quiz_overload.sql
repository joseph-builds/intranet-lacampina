-- Resolve function overloading for create_quiz

-- Drop the old version that had 8 parameters (including p_exam_id)
DROP FUNCTION IF EXISTS public.create_quiz(UUID, TEXT, TEXT, INTEGER, INTEGER, UUID, BOOLEAN, TIMESTAMPTZ);

-- Drop the new version we just created just in case we need to be clean
DROP FUNCTION IF EXISTS public.create_quiz(UUID, TEXT, TEXT, INTEGER, INTEGER, BOOLEAN, TIMESTAMPTZ);

-- Recreate only ONE version of the function without p_exam_id
