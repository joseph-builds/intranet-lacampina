...
interface Student {
  id: string; first_name: string; last_name: string; email: string; dni: string | null; birth_date: string | null;
  phone: string | null; guardian_name: string | null; emergency_phone: string | null;
  role: string; is_active: boolean; current_grade_id?: string; grade?: Grade; section_id?: string; section_name?: string; 
}
...
const AdminStudentManagement = () => {
  ...
  const fetchLevelsAndGrades = async () => {
    ...
    } catch (error) { console.error("Error cargando estructura académica", error); }
  };

  const fetchStudents = async () => {
    try {
      const { data: perfiles, error } = await supabase.from('profiles').select(`id, first_name, last_name, email, dni, birth_date, phone, guardian_name, emergency_phone, is_active, role, current_grade_id, grade:academic_grades(id, name, level:academic_levels(id, name))`).eq('role', 'student').order('first_name', { ascending: true });
      if (error) throw error;
      const { data: matriculas } = await supabase.from('student_sections').select('student_id, section_id, sections(id, name)').eq('academic_year', CURRENT_YEAR);
  ...
  }, [students, searchTerm, filterStatus, nivelFilter, gradoFilter, assignmentFilter]);
  ...
  const currentLimit = itemsPerPage === 'all' ? sortedStudents.length : (itemsPerPage as number);
  const totalPages = Math.max(1, Math.ceil(sortedStudents.length / currentLimit));
  
  const paginatedStudents = useMemo(() => {
    if (itemsPerPage === 'all') return sortedStudents;
    const startIndex = (currentPage - 1) * (itemsPerPage as number);
    return sortedStudents.slice(startIndex, startIndex + (itemsPerPage as number));
  }, [sortedStudents, currentPage, itemsPerPage]);

  useEffect(() => { setCurrentPage(1); }, [searchTerm, filterStatus, nivelFilter, gradoFilter, assignmentFilter, itemsPerPage]);
  ...
  const handleManualPasswordChange = async (studentEmail: string) => {
    if (newManualPassword.length < 6) return toast({ title: "Atención", description: "La contraseña debe tener al menos 6 caracteres.", variant: "destructive" });
    ...
    } catch (error: any) { toast({ title: "Error", description: error.message, variant: "destructive" }); } finally { setIsChangingPassword(false); }
  };
  ...
  const openEditModal = (student: Student) => {
    ...
    setFormData({
      ...
      current_grade_id: student.current_grade_id || 'unassigned', section_id: student.section_id || 'unassigned' 
    });
    setIsModalOpen(true);
  };
  ...
  const handleToggleExemption = async (scId: string | null, isExempted: boolean) => {
    ...
    if (!scId) return toast({ title: "Aula requerida", description: "Para guardar exoneraciones, primero debes asignar una Sección al estudiante.", variant: "destructive" });
    try {
      if (isExempted) {
    ...
    <DashboardLayout>
      <div className="container mx-auto px-4 py-8 space-y-6">
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-6">
          <div>
            <h1 className="text-3xl font-bold flex items-center gap-3 text-gray-800"><Users className="h-8 w-8 text-blue-600" /> Gestión de Estudiantes</h1>
            <p className="text-gray-600 mt-1">Administra cuentas, alumnos, secciones y su matrícula.</p>
          </div>
          <Button onClick={openCreateModal} className="bg-blue-600 hover:bg-blue-700 shadow-md"><Plus className="w-4 h-4 mr-2" />Nuevo Estudiante</Button>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <Card className="hover:shadow-md transition-shadow border-l-4 border-l-blue-500"><div className="p-4"><div className="text-sm font-medium text-gray-600 flex items-center justify-between">Total Registrados <Users className="inline h-5 w-5 text-blue-500" /></div><div className="text-3xl font-bold text-gray-800">{students.length}</div></div></Card>
          <Card className="hover:shadow-md transition-shadow border-l-4 border-l-green-500"><div className="p-4"><div className="text-sm font-medium text-gray-600 flex items-center justify-between">Alumnos Activos <CheckCircle2 className="inline h-5 w-5 text-green-500" /></div><div className="text-3xl font-bold text-gray-800">{students.filter(s => s.is_active).length}</div></div></Card>
          <Card className="hover:shadow-md transition-shadow border-l-4 border-l-red-400"><div className="p-4"><div className="text-sm font-medium text-gray-600 flex items-center justify-between">Desmatriculados <ShieldAlert className="inline h-5 w-5 text-red-400" /></div><div className="text-3xl font-bold text-gray-800">{students.filter(s => !s.is_active).length}</div></div></Card>
        </div>
        ...
        <div className="bg-white p-4 rounded-lg shadow-sm border space-y-4">
            ...
            <div className="flex flex-wrap items-center gap-3 flex-1">
              ...
              <Select value={assignmentFilter} onValueChange={setAssignmentFilter}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="fully_assigned" className="text-green-700">Completos (Grado y Aula)</SelectItem>
                  <SelectItem value="no_grade" className="text-red-600 font-medium">Sin Grado Asignado</SelectItem>
                  <SelectItem value="no_section" className="text-orange-600 font-medium">Con Grado, sin Aula</SelectItem>
                </SelectContent>
              </Select>
            </div>
          ...
          {selectedIds.length > 0 && (
            <div className="bg-blue-50/50 p-3 flex flex-wrap items-center justify-between rounded-md border border-blue-200 animate-in fade-in slide-in-from-top-2">
              <div className="flex items-center gap-2 text-blue-700 font-medium"><CheckSquare className="w-5 h-5" /> {selectedIds.length} alumno(s) seleccionado(s)</div>
        ...
        <Card className="border-0 shadow-sm rounded-lg overflow-hidden">
          <CardContent className="p-0">
              ...
              <div className="overflow-x-auto">
                <Table>
                  <TableHeader className="bg-gray-50/80">
                    <TableRow>
                      <TableHead className="w-12 text-center"><input type="checkbox" className="w-4 h-4 rounded border-gray-300 text-blue-600 cursor-pointer" checked={isAllPageSelected} onChange={(e) => handleSelectAll(e.target.checked)} /></TableHead>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between p-4 bg-white border-t">
              <div className="text-sm text-gray-500 mb-4 sm:mb-0">Mostrando {itemsPerPage === 'all' ? 1 : (currentPage - 1) * (itemsPerPage as number) + 1} - {itemsPerPage === 'all' ? sortedStudents.length : Math.min(currentPage * (itemsPerPage as number), sortedStudents.length)} de {sortedStudents.length} estudiantes</div>
              <div className="flex items-center gap-2">
