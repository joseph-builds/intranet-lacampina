...
export function StudentCourseAttendance({
  courseId,
}: StudentCourseAttendanceProps) {
  ...
  const fetchAttendance = async () => {
    try {
      ...
      const typedRecords: AttendanceRecord[] = (attendanceData || []).map(
        (r) => ({
          id: r.id,
          date: r.date,
      ...
      const justified =
        attendanceData?.filter((r) => r.status === "justified").length || 0;
