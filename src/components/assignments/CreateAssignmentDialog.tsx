  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isDateWithinWeek(formData.due_date)) {
      ...
      if (selectedWeek?.start_date && selectedWeek?.end_date) {
        ...
        toast.error(
          "La fecha límite no está dentro del rango de la semana seleccionada",
        );
      }
      return;
    }
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-2">
            <Label htmlFor="modulo_id" className="required">
              Curso
            </Label>
            {loadingCourses ? (
          ...
          {formData.modulo_id && (
            <div className="space-y-2">
              <Label htmlFor="week_id" className="required">
                Semana del Curso
              </Label>
              {loadingWeeks ? (
                <div className="h-10 bg-muted rounded animate-pulse" />
                ...
                <div className="p-4 bg-muted rounded text-sm text-muted-foreground">
                  Este curso no tiene semanas configuradas. Contacta al
                  administrador.
                </div>
              ) : (
                <>
                  <Select
                    ...
                    onValueChange={(value) =>
                      setFormData((prev) => ({ ...prev, week_id: value }))
                    }
                  >
                    <SelectTrigger>
                        ...
                        <SelectItem key={week.id} value={week.id}>
                          ...
                          {week.start_date && week.end_date && (
                            <span className="text-xs text-muted-foreground ml-2">
                              (
                              {format(new Date(week.start_date), "d MMM", {
                                locale: es,
                              })}{" "}
                              -{" "}
                              {format(new Date(week.end_date), "d MMM", {
                                locale: es,
                              })}
                              )
                            </span>
                  ...
                  {formData.week_id &&
                    getSelectedWeek()?.start_date &&
                    getSelectedWeek()?.end_date && (
                      <p className="text-xs text-muted-foreground">
                        📅 Esta semana va desde el{" "}
                        <strong>
                          {format(
                        ...
                        <strong>
                          {format(
                            new Date(getSelectedWeek()!.end_date!),
                            "d 'de' MMMM",
                            { locale: es },
                          )}
                        </strong>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <Popover>
                ...
                <PopoverContent className="w-auto p-0" align="start">
                  ...
                  <Calendar
                    ...
                    defaultMonth={
                      formData.week_id && getSelectedWeek()?.start_date
                        ? new Date(getSelectedWeek()!.start_date!)
                        : undefined
                    }
                    onSelect={(date) => {
            ...
            <div className="space-y-2">
              ...
              />
              <p className="text-xs text-muted-foreground">
                Esta puntuación es referencial. La calificación final será en
          ...
          <div className="space-y-2">
            ...
            <p className="text-sm text-muted-foreground mb-2">
              Sube documentos, PDFs, imágenes u otros archivos que ayuden a los
              estudiantes
            </p>
